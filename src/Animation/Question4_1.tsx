...
const PPosition = (time: number): { cx: number; cy: number } => {
    ...
    if (time < t[0]) return base;
        ...
        return {
            cx: base.cx + radius + (length * (time - t[1])) / (t[3] - t[1]),
            cy: base.cy + radius,
        };
    else if (t[2] <= time && time < t[3])
        return {
            cx: base.cx + radius + (length * (time - t[1])) / (t[3] - t[1]),
            cy: base.cy + radius + 120 * ((time - t[2]) / (t[3] - t[2])) ** 2,
        };
    else
        return {
            cx: base.cx + radius + length,
            cy: base.cy + radius + 120,
        };
};
const BPosition = (time: number): { x: number; y: number } => {
    ...
    else if (t[1] <= time && time < t[3])
        return {
            x: base.x + (length * (time - t[1])) / (t[3] - t[1]),
            y: base.y,
        };
    else
        return {
...
export const Question4_1 = () => {
    ...
    return (
        <div className={Style.AnimationContainer}>
            ...
            />
            <h1 className={Style.Title}>問4-1</h1>
            <svg
                className={Style.SVG}
